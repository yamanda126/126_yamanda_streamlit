...
if tipe == 'single vector':
    ...
    df1 = pd.DataFrame(columns=range(1, int(col1) + 1), index=range(1, int(row1) + 1), dtype=float)
    st.write('Masukkan data untuk matrix pertama')
    df1_input = st.experimental_data_editor(df1, use_container_width=True, key=1)

    df2 = pd.DataFrame(columns=range(1, int(col2) + 1), index=range(1, int(row2) + 1), dtype=float)
